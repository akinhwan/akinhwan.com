<template>
  ...
  >
    ...
    >
      {{ title }}
      <svg
        class="w-3 h-3 fill-current text-gray-500 cursor-pointer ml-1 flex-shrink-0"
        viewBox="0 0 12 12"
    ...
    >
      <ul
        v-show="dropdownOpen"
        class="origin-top-right absolute top-full right-0 w-40 bg-gray-800 py-2 ml-4 rounded-sm"
      >
        <slot />
      </ul>
    </transition>
